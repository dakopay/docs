dakopay.com is a DeFi payments application which enables trustless recurring payments on different blockchains using ERC20 tokens powered by smart contracts. Merchants can create recurring payment plans in crypto using dakopay.com, where the incurring subscription charges are stored in the smart contract from which they can collect the desired amount from the user for as long as the subscription applies. It is up to merchant to decide the frequency of the recurring payments (daily, weekly or monthly).

dakopay.com has a wide range of use cases; influencers, gamers, NGO’s/charities are now able to use a system for receiving recurring crypto payments.
...
Like any recurring payment solution, even dakopay.com works within boundaries that are defined by mandate parameters. These parameters are as follows:
...
A token address is the address from which payments are withdrawn in order to join a subscription plan. The token address can hold any token supported by DAKOPAY (i.e. any ERC20 token). The users must choose which token to utilize as their cryptocurrency of choice to pay for their subscription plan.
